...
### 🔴 CRITICAL #1: Feed Cache Consistency Race Condition

**Problem**:  
The system uses two cache layers (Redis sorted set + MySQL userFeedCache) but they operate independently:

```typescript
// Line 973: saveRedisFeedRank() - saves 400 items
await this.saveRedisFeedRank(userIdBigInt, ranked);

// Line 471: saveSqlFeedCache() - saves 200 items  
await this.saveSqlFeedCache(userIdBigInt, ranked);
```

**Why It's Critical**:
1. Redis TTL = 90 seconds, SQL TTL = 5 minutes → TTL mismatch
2. When Redis expires, SQL returns stale 5-minute-old feed
3. If invalidation only deletes Redis (line 91), SQL cache becomes zombie data
4. At 10M users with 100 requests/sec, this becomes stale feed serving 99% of the time

**Failure Scenario**:
- User gets feed from Redis (current)
- 90 seconds pass, Redis expires
- User refreshes, gets 4+ minute old feed from SQL
- User is confused: posts they just liked appear again
- Ranking algorithms produce identical scores, feed doesn't change

**Fix**:
```typescript
// 1. Enforce single source of truth - invalidate both atomically
private async invalidateUserFeedCache(userId: string | number | bigint): Promise<number> {
  const userIdBigInt = this.toBigInt(userId);
  const pattern = `feed:user:${userIdBigInt.toString()}:*`;
  
  // ATOMIC: Delete both at the same time, fail if either fails
  const [redisCnt, sqlCnt] = await Promise.all([
  ...
  // Log mismatch (indicates bug in invalidation logic)
  if (redisCnt === 0 && sqlCnt > 0) {
    this.logger.warn(`Cache invalidation mismatch: Redis had 0 keys, SQL had ${sqlCnt}`);
  }

  return Math.max(redisCnt, sqlCnt);
}

// 2. Use same TTL for both (align to Redis, the faster layer)
private async saveRedisFeedRank(userId: bigint, ranked: FeedCandidate[]) {
  ...
  const TTL_SECONDS = 90; // Match Redis TTL
  
  await this.redisService.replaceSortedSet(rankKey, entries, TTL_SECONDS);
}

private async saveSqlFeedCache(userId: bigint, ranked: FeedCandidate[]) {
  const topCandidates = ranked.slice(0, 200);
  
  // CRITICAL: Set createdAt to NOW so TTL check works correctly
  const now = new Date();
  ...
  await this.prisma.userFeedCache.createMany({
    data: topCandidates.map((item) => ({
      ...
      postId: item.postId,
      score: item.score,
      createdAt: now, // Consistent timestamp for TTL
    })),
    skipDuplicates: true,
  });
}

// 3. Update cache lookup to use consistent TTL
private async getSqlFeedCache(
  ...
  // Use Redis TTL (90 seconds) not 5 minutes
  const freshnessCutoff = new Date(Date.now() - 90 * 1000);
  // ... rest unchanged
}
```
